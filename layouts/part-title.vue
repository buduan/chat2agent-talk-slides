<template>
  <div class="slidev-layout part-title">
    ...
    <div class="part-title-content">
      ...
      <div class="part-number">
        <div class="part-number-text">Part {{ $frontmatter.part || 1 }}</div>
      </div>
      ...
      <div class="part-subtitle" :style="{ animationDelay: '600ms' }">
        {{ $frontmatter.partSubtitle || '' }}
      </div>

      <!-- Optional description -->
      <div v-if="$frontmatter.partDescription" class="part-description" :style="{ animationDelay: '800ms' }">
        {{ $frontmatter.partDescription }}
      </div>
    </div>
    ...
    <div class="part-progress">
      <div class="progress-bar">
        <div
          class="progress-fill"
          :style="{ width: `${(($frontmatter.part || 1) / ($frontmatter.totalParts || 4)) * 100}%` }"
        ></div>
      </div>
      <div class="progress-text">{{ $frontmatter.part || 1 }} / {{ $frontmatter.totalParts || 4 }}</div>
    </div>
  </div>
</template>
...
const titleWords = computed(() => {
  return ($frontmatter.partTitle || 'Part Title').split(' ')
})
</script>
